...
Eval compute in ofst None.
Eval compute in ofst (Some (4, 5)).
...
:=
  match l with
  | nil => None
  | (key, value) :: xs => if key == y then Some value else find y xs
  end.
...
Definition square_cache := map (fun x => (x, x*x)) data.
...
Definition square cache n :=
  ...
  | None => let sq := n * n in (sq, (n, sq) :: cache)
  | Some sq => (sq, cache)
  end.
